...
def application(environ: dict, start_response: Callable) -> list[bytes]:
    ...
    if route_match:
        ...
        if method == "POST" and action == "acknowledge":
            try:
                return _json_response(start_response, 200, acknowledge_recommendation(store, recommendation_id).to_dict())
            except KeyError:
                return _json_response(start_response, 404, {"error": "recommendation not found"})
        if method == "POST" and action == "suppress":
            try:
                return _json_response(start_response, 200, suppress_recommendation(store, recommendation_id, "suppressed via API").to_dict())
            except KeyError:
                return _json_response(start_response, 404, {"error": "recommendation not found"})
...
def _match_recommendation_route(path: str) -> tuple[str, str | None] | None:
    ...
    if len(parts) == 1:
        return parts[0], None
    if len(parts) == 2 and parts[1] in {"acknowledge", "suppress"}:
